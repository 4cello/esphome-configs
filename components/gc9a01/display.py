    ...
    GC9A01_SCHEMA.extend(
        {
            ...
            cv.Optional(CONF_DEVICE_WIDTH): cv.int_,
            cv.Optional(CONF_DEVICE_HEIGHT): cv.int_,
            cv.Optional(CONF_OFFSET_Y, default=0): cv.int_,
            cv.Optional(CONF_OFFSET_X, default=0): cv.int_,
            cv.Optional(CONF_EIGHT_BIT_COLOR, default=True): cv.boolean, # changed this to True, because ain't nobody got heap for that
